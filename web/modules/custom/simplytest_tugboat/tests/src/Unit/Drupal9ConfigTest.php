        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd stm && composer require drupal/token:1.9 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer config --no-interaction allow-plugins.cweagans/composer-patches true',
              'cd stm && composer config --no-interaction extra.composer-patches.patches-file patches.json',
              'cd stm && composer require --no-update cweagans/composer-patches:^2.0',
              'cd stm && echo \'{"patches":{"drupal/token":[{"description":"STM patch 3185080-3.patch","url":"https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch","depth":1,"extra":{"freeform":{"executable":"patch","dry_run_args":"-p%s -d %s --dry-run --no-backup-if-mismatch -i %s","args":"-p%s -d %s --no-backup-if-mismatch -i %s"}}}]}}\' > patches.json',
              'cd stm && composer update --no-ansi -v',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si standard --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
              'cd "${DOCROOT}" && ../vendor/bin/drush config-set system.logging error_level verbose -y',
              'cd "${DOCROOT}" && ../vendor/bin/drush en token -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.3.2',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/panopoly:2.0-alpha15 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.3.2',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/core:9.3.2 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer config --no-interaction allow-plugins.cweagans/composer-patches true',
              'cd stm && composer config --no-interaction extra.composer-patches.patches-file patches.json',
              'cd stm && composer require --no-update cweagans/composer-patches:^2.0',
              'cd stm && echo \'{"patches":{"drupal/core":[{"description":"STM patch 3185080-3.patch","url":"https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch","depth":2,"extra":{"freeform":{"executable":"patch","dry_run_args":"-p%s -d %s --dry-run --no-backup-if-mismatch -i %s","args":"-p%s -d %s --no-backup-if-mismatch -i %s"}}}]}}\' > patches.json',
              'cd stm && composer update --no-ansi -v',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si demo_umami --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
              'cd "${DOCROOT}" && ../vendor/bin/drush config-set system.logging error_level verbose -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.3.2',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/password_policy:3.0-beta1 --no-install',
              'cd stm && composer require drupal/password_policy_pwned:1.0-beta2 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si standard --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'command -v yq > /dev/null || (wget -q https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64 -O /usr/local/bin/yq && chmod +x /usr/local/bin/yq)',
              'composer config --global policy.advisories.block false',
              '[ "$(cd "${TUGBOAT_ROOT}/stm" 2>/dev/null && composer show drupal/core --format=json 2>/dev/null | jq -r \'.versions[0]\')" = "9.1.5" ] && echo "Reusing Drupal 9.1.5 from the base preview" || (rm -rf "${DOCROOT}" "${TUGBOAT_ROOT}/stm" && cd "${TUGBOAT_ROOT}" && composer -n create-project drupal/recommended-project:9.1.5 stm --no-install && cd "${TUGBOAT_ROOT}/stm" && composer config minimum-stability dev && cd "${TUGBOAT_ROOT}/stm" && composer config prefer-stable true && cd "${TUGBOAT_ROOT}/stm" && composer require --no-install drush/drush && ln -snf "${TUGBOAT_ROOT}/stm/web" "${DOCROOT}")',
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.1.5',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/token:1.9 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'command -v yq > /dev/null || (wget -q https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64 -O /usr/local/bin/yq && chmod +x /usr/local/bin/yq)',
              'composer config --global policy.advisories.block false',
              '[ "$(cd "${TUGBOAT_ROOT}/stm" 2>/dev/null && composer show drupal/core --format=json 2>/dev/null | jq -r \'.versions[0]\')" = "9.3.x-dev" ] && echo "Reusing Drupal 9.3.x-dev from the base preview" || (rm -rf "${DOCROOT}" "${TUGBOAT_ROOT}/stm" && cd "${TUGBOAT_ROOT}" && composer -n create-project drupal/recommended-project:9.3.x-dev stm --no-install && cd "${TUGBOAT_ROOT}/stm" && composer config minimum-stability dev && cd "${TUGBOAT_ROOT}/stm" && composer config prefer-stable true && cd "${TUGBOAT_ROOT}/stm" && composer require --no-install drush/drush && ln -snf "${TUGBOAT_ROOT}/stm/web" "${DOCROOT}")',
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.3.x-dev',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/core:9.3.x-dev --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si standard --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'command -v yq > /dev/null || (wget -q https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64 -O /usr/local/bin/yq && chmod +x /usr/local/bin/yq)',
              'composer config --global policy.advisories.block false',
              '[ "$(cd "${TUGBOAT_ROOT}/stm" 2>/dev/null && composer show drupal/core --format=json 2>/dev/null | jq -r \'.versions[0]\')" = "9.1.9" ] && echo "Reusing Drupal 9.1.9 from the base preview" || (rm -rf "${DOCROOT}" "${TUGBOAT_ROOT}/stm" && cd "${TUGBOAT_ROOT}" && composer -n create-project drupal/recommended-project:9.1.9 stm --no-install && cd "${TUGBOAT_ROOT}/stm" && composer config minimum-stability dev && cd "${TUGBOAT_ROOT}/stm" && composer config prefer-stable true && cd "${TUGBOAT_ROOT}/stm" && composer require --no-install drush/drush && ln -snf "${TUGBOAT_ROOT}/stm/web" "${DOCROOT}")',
              'cd "${TUGBOAT_ROOT}/stm" && composer require --dev --no-install drupal/core:9.1.9',
              'echo "SIMPLYEST_STAGE_DOWNLOAD"',
              'cd stm && composer require drupal/core:9.1.9 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si standard --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd stm && composer require drupal/core:9.3.x-dev --no-install',
              'cd stm && composer require drupal/token:1.9 --no-install',
              'cd stm && composer require drupal/uswds:2.4 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd stm && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'cd "${DOCROOT}" && ../vendor/bin/drush si standard --db-url=mysql://tugboat:tugboat@mysql:3306/tugboat --account-name=admin --account-pass=admin -y',
              'cd "${DOCROOT}" && ../vendor/bin/drush config-set system.logging error_level verbose -y',
              'cd "${DOCROOT}" && ../vendor/bin/drush en token -y',
              'deps=$(yq -o=json \'.dependencies // []\' ${DOCROOT}/themes/contrib/uswds/uswds.info.yml | jq -r \'.[] | split(":")[1]\' | xargs); [ -z "$deps" ] || ${DOCROOT}/../vendor/bin/drush en $deps -y',
