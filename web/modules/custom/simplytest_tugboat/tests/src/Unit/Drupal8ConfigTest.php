        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer patch-add drupal/token "STM patch 3185080-3.patch" "https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch" --no-update',
              'cd "${DOCROOT}" && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
              'drush -r "${DOCROOT}" config-set system.logging error_level verbose -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'composer global require szeidler/composer-patches-cli:~1.0',
              'cd "${DOCROOT}" && composer patch-enable --file="patches.json"',
              'cd "${DOCROOT}" && composer patch-add drupal/core "STM patch 3185080-3.patch" "https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch" --no-update',
              'cd "${DOCROOT}" && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
              'drush -r "${DOCROOT}" config-set system.logging error_level verbose -y',
