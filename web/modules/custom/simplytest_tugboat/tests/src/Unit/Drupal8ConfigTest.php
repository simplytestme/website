        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer require zaporylie/composer-drupal-optimizations:^1.0 --no-install',
              'cd "${DOCROOT}" && composer install --no-ansi',
              'cd "${DOCROOT}" && composer require drupal/token:1.9 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd "${DOCROOT}" && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
              'drush -r "${DOCROOT}" config-set system.logging error_level verbose -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer require drupal/token:1.9 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd "${DOCROOT}" && composer config --no-interaction allow-plugins.cweagans/composer-patches true',
              'cd "${DOCROOT}" && composer config --no-interaction extra.composer-patches.patches-file patches.json',
              'cd "${DOCROOT}" && composer require --no-update cweagans/composer-patches:^2.0',
              'cd "${DOCROOT}" && echo \'{"patches":{"drupal/token":[{"description":"STM patch 3185080-3.patch","url":"https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch","depth":1,"extra":{"freeform":{"executable":"patch","dry_run_args":"-p%s -d %s --dry-run --no-backup-if-mismatch -i %s","args":"-p%s -d %s --no-backup-if-mismatch -i %s"}}}]}}\' > patches.json',
              'cd "${DOCROOT}" && composer update --no-ansi -v',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
              'drush -r "${DOCROOT}" config-set system.logging error_level verbose -y',
              'drush -r "${DOCROOT}" en token -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer require zaporylie/composer-drupal-optimizations:^1.0 --no-install',
              'cd "${DOCROOT}" && composer install --no-ansi',
              'cd "${DOCROOT}" && composer require drupal/panopoly:2.0-alpha15 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd "${DOCROOT}" && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer config --no-interaction extra.composer-patches.patches-file patches.json',
              'cd "${DOCROOT}" && composer require --no-update cweagans/composer-patches:^2.0',
              'cd "${DOCROOT}" && echo \'{"patches":{"drupal/core":[{"description":"STM patch 3185080-3.patch","url":"https://www.drupal.org/files/issues/2020-12-07/3185080-3.patch","depth":2,"extra":{"freeform":{"executable":"patch","dry_run_args":"-p%s -d %s --dry-run --no-backup-if-mismatch -i %s","args":"-p%s -d %s --no-backup-if-mismatch -i %s"}}}]}}\' > patches.json',
              'cd "${DOCROOT}" && composer update --no-ansi -v',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
              'drush -r "${DOCROOT}" config-set system.logging error_level verbose -y',
        ...
        'php' => [
          ...
          'commands' => [
            'build' => [
              ...
              'cd "${DOCROOT}" && composer require zaporylie/composer-drupal-optimizations:^1.0 --no-install',
              'cd "${DOCROOT}" && composer install --no-ansi',
              'cd "${DOCROOT}" && composer require drupal/password_policy:3.0-beta1 --no-install',
              'cd "${DOCROOT}" && composer require drupal/password_policy_pwned:1.0-beta2 --no-install',
              'echo "SIMPLYEST_STAGE_PATCHING"',
              'cd "${DOCROOT}" && composer update --no-ansi',
              'echo "SIMPLYEST_STAGE_INSTALLING"',
              'drush -r "${DOCROOT}" si standard --account-name=admin --account-pass=admin -y',
