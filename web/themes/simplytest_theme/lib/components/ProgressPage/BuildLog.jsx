...
function BuildLog({ logs, open, onToggle }) {
  const text = logs.map(log => (log.message || "").replace(/^\s+|\s+$/g, "")).join("\n");
  return (
    <div className="overflow-hidden rounded-xl border border-st-line">
      ...
      >
        <span className="flex items-center gap-2.5">
          ...
          <span className="font-mono text-[11px] text-st-faint">{logs.length} lines</span>
        </span>
        <span className="flex gap-4 text-[13px] font-semibold text-st-accent-dark">
          ...
          <button type="button" className="hover:text-st-accent" onClick={onToggle}>
            {open ? "Hide ▴" : "Show ▾"}
          </button>
        </span>
      </div>
